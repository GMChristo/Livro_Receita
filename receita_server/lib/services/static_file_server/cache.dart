...
import 'my_virtual_directory.dart';
...
class CachingVirtualDirectory extends MyVirtualDirectory {
  ...
  Future<bool> serveFile(File file, FileStat stat, RequestContext req, ResponseContext res) {
    ...
    if (shouldNotCache) {
      ...
      if (ifModified != null) {
        try {
          final ifModifiedSince = ifModified;
  ...
  void setCachedHeaders(DateTime modified, RequestContext req, ResponseContext res) {
    final privacy = accessLevelToString(accessLevel ?? CacheAccessLevel.public);
    ...
    if (maxAge != null) {
      final expiry = DateTime.now().add(Duration(seconds: maxAge ?? 0));
