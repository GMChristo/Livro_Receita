...
class MyVirtualDirectory {
  MyVirtualDirectory(this.app, this.fileSystem,
      {Directory source,
    ...
    if (source != null) {
      ...
      final String dirPath = app.environment.isProduction ? './build/web' : './web';
  ...
  Future<bool> handleRequest(RequestContext req, ResponseContext res) {
    ...
    final path = req.uri.path.replaceAll(_straySlashes, '');
  ...
  RequestHandler pushState(String path, {Iterable accepts}) {
    ...
    return (RequestContext req, ResponseContext res) {
      final path = req.path.replaceAll(_straySlashes, '');
      if (path == vPath) {
        return Future<bool>.value(true);
      }
  ...
  /// Writes the file at the given virtual [_path] to a response.
  Future<bool> servePath(String _path, RequestContext req, ResponseContext res) async {
    String path = _path;
    ...
    final absolute = source.absolute.uri.resolve(path).toFilePath();
    final parent = source.absolute.uri.toFilePath();

    if (!p.isWithin(parent, absolute) && !p.equals(parent, absolute)) {
      return true;
    }

    final stat = await fileSystem.stat(absolute);
  ...
  Future<bool> serveStat(
    ...
    if (stat.type == FileSystemEntityType.directory) {
      ...
      final link = fileSystem.link(absolute);
  ...
  Future<bool> serveDirectory(
    ...
    if (allowDirectoryListing == true) {
      ...
      final List<FileSystemEntity> entities =
          await directory.list(followLinks: false).toList().then((l) => List.from(l));
      entities.sort((a, b) {
        if (a is Directory) {
          if (b is Directory) {
      ...
      for (var entity in entities) {
        final stub = p.basename(entity.path);
        var href = stub;
        String type;
  ...
  void _ensureContentTypeAllowed(String mimeType, RequestContext req) {
    final value = req.headers.value('accept');
    final bool acceptable = value == null ||
  ...
  Future<bool> serveFile(File file, FileStat stat, RequestContext req, ResponseContext res) async {
    ...
    if (callback != null) {
      await req.app.executeHandler((RequestContext req, ResponseContext res) => callback(file, req, res), req, res);
    }

    final type = app.mimeTypeResolver.lookup(file.path) ?? 'application/octet-stream';
    res.headers['accept-ranges'] = 'bytes';
    ...
    if (req.headers.value('range')?.startsWith('bytes=') != true) {
      ...
      final totalFileSize = await file.length();
      ...
      if (header.items.isEmpty) {
        ...
        final item = header.items[0];
        Stream<List<int>> stream;
        int len = 0;
        final int total = totalFileSize;
        ...
        final transformer = RangeHeaderTransformer(
