...
// import 'package:visitacao_browser/src/shared/exceptions/no_content_exception.dart';
// import 'package:visitacao_core/visitacao_core.dart';
...
class BackendRepository {
  ...
  Future<List<Map<String, dynamic>>> getAllAsMap(String collection, {a}) async {
  ...
  Future<List<Map<String, dynamic>>> getMenuHierarquia(
    ...
    if (maps?.isNotEmpty == true) {
      ...
      if (filtrarPorTipos?.isNotEmpty == true) {
        roots =
            roots.where((m) => filtrarPorTipos.contains(m['tipo'])).toList();
      }
  ...
  //int count = 0;
  List<Map<String, dynamic>> _getMenuChildren(List<Map<String, dynamic>> maps,
      String idPai, int level, List<String> filtrarPorTipos) {
  ...
  Future<List<Map<String, dynamic>>> getAllMidiasAsMap({a}) async {
    var client = http.Client();
    var list = <Map<String, dynamic>>[];
  ...
  Future<Map<String, dynamic>> getByIdAsMap(
  ...
  Future<Map<String, dynamic>> findFirstByAsMap(String collection,
    ...
    var url =
        '${AppConfig().serverURL}/rest/findFirstBy?collection=$collection';
    if (queryMap != null) {
  ...
  Future<void> createAsMap(Map<String, dynamic> map, String collection) async {
    ...
    var resp = await client.post(
      Uri.parse('${AppConfig().serverURL}/rest?collection=$collection'),
      headers: AppConfig().defaultHeaders,
      body: jsonEncode(map),
    );
  ...
  //   var client = http.Client();
  //   var resp = await client.post(
  //     Uri.parse('${AppConfig().serverURL}/auth/usuario'),
  //       headers: AppConfig().defaultHeaders,
  //       body: jsonEncode(usuario.getMapForSave()),
  //       );
  ...
  //     Uri.parse('${AppConfig().serverURL}/auth/usuario/by/username'),
  //       headers: AppConfig().defaultHeaders,
  //       body: jsonEncode(usuario.getMapForSave()),
  //       );
  ...
  //   var client = http.Client();
  //   var resp = await client.put(
  //     Uri.parse('${AppConfig().serverURL}/auth/usuario/by/ide'),
  //       headers: AppConfig().defaultHeaders,
  //       body: jsonEncode(usuario.getMapForSave()),
  //       );
  ...
  Future<void> deleteMidiaById(String id) async {
    ...
    var resp = await client.delete(
      Uri.parse('${AppConfig().serverURL}/midias/$id'),
      headers: AppConfig().defaultHeaders,
    );
  ...
  Future<void> deleteAsMap(Map<String, dynamic> map, String collection) async {
    ...
    var resp = await client.delete(
      Uri.parse(
          '${AppConfig().serverURL}/rest?collection=$collection&filterByMap=$filterByMap'),
      headers: AppConfig().defaultHeaders,
    );
  ...
  Future<void> updateByMap(Map<String, dynamic> byMap,
  ...
  //   var itens;
  //   var resp = await client.get(
  //     Uri.parse('${AppConfig().serverURL}/rest?collection=$collection&limit=1&sortBy=dataCadastro&sortDir=true'),
  //     headers: AppConfig().defaultHeaders,
  //   );
  //   if (resp.statusCode == 200) {
